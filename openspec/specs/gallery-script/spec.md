...
Defines the extracted `initGallery` factory module and the contract for photo gallery initialisation. Gallery logic lives in `src/scripts/gallery.ts`; `PhotoGallery.astro`'s `<script>` block is a minimal factory call.
